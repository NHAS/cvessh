...
func (cb publicKeyCallback) auth(session []byte, user string, c packetConn, rand io.Reader, extensions map[string][]byte) (authResult, []string, error) {
	...
	for idx := 0; idx < len(signers); idx++ {
		...
		var newSig sig
		signer := signers[idx]
		newSig.pub = signer.PublicKey()

		newSig.as, newSig.algo, err = pickSignatureAlgorithm(signer, extensions)
		...
		ok, err := validateKey(newSig.pub, newSig.algo, user, c)
		if err != nil {
			return authFailure, nil, err
		}
		...
		if !ok && idx < origSignersLen && isRSACert(newSig.algo) && newSig.algo != CertAlgoRSAv01 {
			if contains(newSig.as.Algorithms(), KeyAlgoRSA) {
				// We retry using the compat algorithm after all signers have
				// been tried normally.
				signers = append(signers, &multiAlgorithmSigner{
					AlgorithmSigner:     newSig.as,
		...
		if ok {
			valids = append(valids, newSig)
		}
	}

	for _, v := range valids {
		pubKey := v.pub.Marshal()
		data := buildDataSignedForAuth(session, userAuthRequestMsg{
			User:    user,
			Service: serviceSSH,
			Method:  cb.method(),
		}, v.algo, pubKey)
		sign, err := v.as.SignWithAlgorithm(rand, data, underlyingAlgo(v.algo))
		if err != nil {
			return authFailure, nil, err
		}
